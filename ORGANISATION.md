...
Dans le workspace organisé, ouvrir **`actifs/fireviewer/fireviewer-evidence-supervisor`**. Chaque dossier est un dépôt Git autonome. Créer une branche de chantier dans ce dépôt ; pour un travail parallèle, créer un worktree sous `travaux/<chantier>/<composant>`. Ne jamais commiter depuis la racine multi-dépôts, `archives/` ou un ancien chemin de compatibilité.

Règles et contrats de supervision des preuves.

Décisions humaines, révisions et publication durable restent au backend.
